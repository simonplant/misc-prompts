...
```mermaid
graph TD
    A[Multiple Agent Responses] --> B{What's your primary need?}
    B -->|Quick synthesis<br/>Standard complexity| C[Tool 1: Rapid Synthesis]
    B -->|Strategic insight<br/>Hidden patterns| D[Tool 2: Deep Analysis]
    B -->|Stakeholder ready<br/>Polished output| E[Tool 3: Publication-Ready]
    C --> F{Quality sufficient?}
    F -->|Yes| G[Done]
    F -->|Needs polish| E
    F -->|Needs depth| D
    D --> H{Ready for distribution?}
    H -->|Yes| E
    H -->|No| I[Use as strategic input]
    
    J[Versioned Content?] -->|Yes| K[Add Temporal Module]
    K --> B
```
