---
name: content-pitch
description: "Creates and refines technical article pitch blocks from raw input (conversations, build logs, decisions, rants). Uses a six-critic loop (insight, so-what, novelty, grounding, self-promotion, structure) and thinking toolkit techniques to sharpen pitches. Supports Audit Mode for evaluating existing articles and Series Mode for analyzing multi-article sequences. Use whenever the user wants to brainstorm, pitch, or workshop article ideas, audit existing content, evaluate a series, or says things like 'this could be a post', 'article idea', 'pitch this', 'review this article', 'audit this content', or 'evaluate this series'. Also triggers on raw notes, build logs, or decision records the user wants to mine for publishable ideas."
---
